...
import com.amgreat.integrator.be.CacheIntegrator;
import com.amgreat.integrator.be.DataIntegrator;
...
public class LoadTableMapping2Cache {
	...
	public boolean loadTableColumnsMapping( String tableId, String tableName ) {
			...
			req.setColumnName("a.t_col_id");req.setName("__t_col_id");req.setValue(""); req.setType("str"); req = req.setNext(new AttributeVO());
			...
			if( r != null ) {
				...
				while( result != null && result.getResponse() != null) 
				{
					...
					tcache.setLabel( this.getColumAt( columns, 3) );
					...
					tcache.setTypeId( this.getColumAt(columns, 7) );
